...
환경변수 (Railway에서 설정):
  ...
  ALLOWED_USER_ID  - 내 텔레그램 user_id (보안용, 선택사항)
"""
        ...
        try:
            ...
            if success:
                await app.bot.send_message(chat_id,
                    f"🎉 *취소표 예매 성공!*\n{d['dep']} → {d['arr']} {d['date']}\n"
                    f"자세한 내용은 앱에서 확인하세요.", parse_mode="Markdown")
                app.bot_data.get("tasks", {}).pop(chat_id, None)
                return
            else:
                if attempt % 10 == 0:  # 5분마다 상태 보고
                    await app.bot.send_message(chat_id,
                        f"🔍 조회 중... ({attempt}회 시도, 아직 취소표 없음)")
        except asyncio.CancelledError:
            return
        except Exception as e:
            log.error(f"매크로 오류: {e}")
            await app.bot.send_message(chat_id, f"⚠️ 오류 발생: {e}\n계속 재시도합니다.")

        await asyncio.sleep(interval)


def try_reserve(d: dict) -> bool:
    """실제 예매 시도 (동기 함수, 스레드에서 실행)"""
    if d["rail"] == "SRT":
        return _try_srt(d)
    else:
        return _try_ktx(d)
